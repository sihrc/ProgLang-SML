  ...
  (case prompt () 
    ...
    and eval_print fenv str = 
      let val ts = lexString str in 
        (case parse_wdef ts of 
          DeclExpression(e) => (
          let val _ = pr (["Tokens ="] @ (map stringOfToken ts))
              val _ = pr ["Internal rep = ", stringOfExpr (e)]
              val v = eval fenv e
              val _ = pr [stringOfValue v] in read fenv end)
        |DeclDefinition (str, params, e) => (
           let val _ = pr (["Tokens ="] @ (map stringOfToken ts))
               in read ((str, FDef(params,e))::fenv) end)
              ) end
   handle Parsing msg => (pr ["Parsing error:", msg]; read fenv)
        | Evaluation msg => (pr ["Evaluation error:", msg]; read fenv)
in
